...
@Directive({
  ...
  selector: '[eniResizable]',
  exportAs: 'eniResizable'
})
export class ResizableDirective implements OnInit, OnChanges, OnDestroy, AfterViewInit {
  ...
  public removeStyles() {
    ...
    this.renderer.setStyle(container, 'left', '');
    this.renderer.setStyle(container, 'top', '');
  }
  ...
  private updateResizable() {
    ...
    // clear handles:
    this.renderer.removeClass(element, 'eni-resizable');
    this.removeHandles();

    // create new ones:
    if (this._resizable) {
      this.renderer.addClass(element, 'eni-resizable');
      this.createHandles();
    }
  }
  ...
  private updateAspectRatio() {
    if (typeof this.rzAspectRatio === 'boolean') {
      ...
      const r = Number(this.rzAspectRatio);
  ...
  private createHandles() {
    ...
    if (typeof this.rzHandles === 'string') {
      ...
      for (const type of tmpHandleTypes) {
        // default handle theme: eni-resizable-$type.
        const handle = this.createHandleByType(type, `eni-resizable-${type}`);
        if (handle) {
  ...
  private createHandleByType(type: string, css: string): ResizeHandle {
    ...
    return new ResizeHandle(_el, this.renderer, type, css, this.onMouseDown.bind(this), this.ngZone);
  }
  ...
  private checkBounds() {
    if (this._containment) {
      const maxWidth =
        this._bounding.width - this._bounding.pr - this._bounding.deltaL - this._bounding.translateX - this._currPos.x;
      const maxHeight =
        this._bounding.height - this._bounding.pb - this._bounding.deltaT - this._bounding.translateY - this._currPos.y;
