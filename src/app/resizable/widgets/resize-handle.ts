...
export class ResizeHandle {
  ...
  constructor(
    ...
    public type: string,
    public css: string,
    private onMouseDown: (event: MouseEvent | TouchEvent, handle: ResizeHandle) => void,
    private ngZone: NgZone
  ) {
    // generate handle div
    const handle = renderer.createElement('div');
    renderer.addClass(handle, 'eni-resizable-handle');
    renderer.addClass(handle, css);

    // add default diagonal for se handle
    if (type === 'se') {
      renderer.addClass(handle, 'eni-resizable-diagonal');
    }
